...
const IncentivesAndPenalties: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch incentives and penalties from API
      const [incentivesResponse, penaltiesResponse] = await Promise.all([
        apiGet('/incentives/rewards'),
        apiGet('/incentives/penalties')
      ]);

      setIncentives(incentivesResponse.data || []);
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `incentives-export-${new Date().toISOString().split('T')[0]}.csv`;
                    ...
                    <tr key={incentive.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={async () => {
                              const points = prompt('Update points', String(incentive.points));
                              if (!points) return;
                              try {
                                // Updating incentives may be domain-specific; here we create an additional award to adjust
                                await apiPost('/incentives/citizen/award', { citizenId: incentive.recipientId, points: Number(points) - incentive.points, reason: 'Adjustment', category: 'participation' });
                                await fetchData();
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                // Soft-delete not provided; skip to avoid data loss
                                alert('Delete not supported for incentives in this build');
                              } catch (e) { console.error(e); }
                            }}
                    ...
                    <tr key={penalty.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={async () => {
                              const amount = prompt('Update amount', String(penalty.amount));
                              if (!amount) return;
                              try {
                                alert('Amount updates should be handled via cancel + re-issue in admin backend');
                              } catch (e) { console.error(e); }
                            }}
                            className="text-indigo-600 hover:text-indigo-900"
                          >
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                alert('Delete not supported for penalties in this build');
                              } catch (e) { console.error(e); }
                            }}
                            className="text-red-600 hover:text-red-900"
                          >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">{activeTab === 'incentives' ? 'Award Incentive' : 'Impose Penalty'}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input value={form.citizenId} onChange={(e) => setForm({ ...form, citizenId: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                {activeTab === 'incentives' ? (
                  <>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Points</label>
                      <input type="number" value={form.points} onChange={(e) => setForm({ ...form, points: Number(e.target.value) })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Reason</label>
                      <input value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                    </div>
                  </>
                ) : (
                  <>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Violation Type</label>
                      <select value={form.violationType} onChange={(e) => setForm({ ...form, violationType: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Amount</label>
                      <input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Description</label>
                      <input value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      setCreating(true);
                      if (activeTab === 'incentives') {
                        await apiPost('/incentives/citizen/award', { citizenId: form.citizenId, points: form.points, reason: form.reason, category: 'participation' });
                      } else {
                        const token = localStorage.getItem('token') || '';
                        const resp = await fetch('/api/penalties/impose', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ citizenId: form.citizenId, violationType: form.violationType, amount: form.amount, description: form.reason }) });
                        if (!resp.ok) throw new Error('Penalty creation failed');
                      }
                      setShowAddModal(false);
                      setForm({ mode: 'incentive', citizenId: '', points: 0, reason: '', violationType: 'non_segregation', amount: 0 });
                      await fetchData();
                    } catch (e) {
                ...
                >{creating ? 'Saving...' : (activeTab === 'incentives' ? 'Award Points' : 'Impose Penalty')}</button>
              </div>
            </div>
          </div>
        </div>
