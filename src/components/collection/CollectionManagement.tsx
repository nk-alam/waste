...
const CollectionManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch vehicles and pickup records from API
      const [vehiclesResponse, pickupsResponse] = await Promise.all([
        apiGet('/collection/vehicles'),
        apiGet('/collection/pickups')
      ]);

      setVehicles(vehiclesResponse.data || []);
      setPickupRecords(pickupsResponse.data || []);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          ...
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'pickups'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
                    ...
                    <tr key={vehicle.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={async () => {
                              const number = prompt('Update vehicle number', vehicle.vehicleNumber);
                              if (!number) return;
                              try {
                                await fetch(`/api/collection/vehicles/${vehicle.id}`, {
                                  method: 'PUT',
      ...
      {showAddModal && activeTab === 'vehicles' && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Add Collection Vehicle</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Vehicle Number</label>
                  <input value={vehicleForm.vehicleNumber} onChange={(e) => setVehicleForm({ ...vehicleForm, vehicleNumber: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Type</label>
                  <select value={vehicleForm.type} onChange={(e) => setVehicleForm({ ...vehicleForm, type: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2">
                ...
                <div>
                  ...
                  <input type="number" value={vehicleForm.capacity} onChange={(e) => setVehicleForm({ ...vehicleForm, capacity: Number(e.target.value) })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Area</label>
                  <input value={vehicleForm.area} onChange={(e) => setVehicleForm({ ...vehicleForm, area: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Driver Name</label>
                  <input value={vehicleForm.driverName} onChange={(e) => setVehicleForm({ ...vehicleForm, driverName: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Driver License</label>
                  <input value={vehicleForm.driverLicense} onChange={(e) => setVehicleForm({ ...vehicleForm, driverLicense: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Driver Phone</label>
                  <input value={vehicleForm.driverPhone} onChange={(e) => setVehicleForm({ ...vehicleForm, driverPhone: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
              </div>
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      setCreating(true);
                      await apiPost('/collection/vehicles/register', {
                        vehicleNumber: vehicleForm.vehicleNumber,
                        type: vehicleForm.type,
                        capacity: vehicleForm.capacity,
                        driver: { name: vehicleForm.driverName, license: vehicleForm.driverLicense, phone: vehicleForm.driverPhone },
                        area: vehicleForm.area
                      });
                      setShowAddModal(false);
                      setVehicleForm({ vehicleNumber: '', type: 'truck', capacity: 1000, driverName: '', driverLicense: '', driverPhone: '', area: '' });
                      await fetchData();
                    } catch (e) {
                ...
                >{creating ? 'Saving...' : 'Save Vehicle'}</button>
              </div>
            </div>
          </div>
        </div>
