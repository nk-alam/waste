...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const [wasteGen, collectionEff, facilitiesUtil, penaltiesRev, incentivesDist] = await Promise.all([
          apiGet('/analytics/waste-generation/daily?days=14'),
          apiGet('/analytics/collection/efficiency?days=30'),
          apiGet('/analytics/facilities/utilization?days=30'),
          apiGet('/analytics/penalties/revenue?days=30'),
          apiGet('/analytics/incentives/distribution?days=30')
        ]);

        const totalWasteCollected = wasteGen?.data?.summary?.totalWaste || 0;
        const totalFacilities = facilitiesUtil?.data?.summary?.totalFacilities || 0;
        const activeVehicles = (collectionEff?.data?.vehiclePerformance || []).filter((v: any) => v.totalPickups > 0).length;
        const totalReports = (wasteGen?.data?.summary?.totalPickups || 0) + (collectionEff?.data?.summary?.totalComplaints || 0);
        const totalIncentives = incentivesDist?.data?.summary?.totalIncentives || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="space-y-4">
          {[
            { id: 1, action: 'New citizen registered', time: '2 minutes ago', icon: Users, color: 'bg-blue-500' },
            { id: 2, action: 'Waste collection completed', time: '5 minutes ago', icon: Trash2, color: 'bg-green-500' },
            { id: 3, action: 'Violation reported', time: '10 minutes ago', icon: AlertTriangle, color: 'bg-red-500' },
            { id: 4, action: 'Training completed', time: '15 minutes ago', icon: CheckCircle, color: 'bg-purple-500' },
          ].map((activity) => {
