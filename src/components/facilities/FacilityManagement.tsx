...
const FacilityManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch facilities and waste intakes from API
      const [facilitiesResponse, intakesResponse] = await Promise.all([
        apiGet('/facilities'),
        apiGet('/facilities/intakes')
      ]);

      setFacilities(facilitiesResponse.data || []);
      setWasteIntakes(intakesResponse.data || []);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          ...
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'intakes'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
                    ...
                    <tr key={facility.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={async () => {
                              const name = prompt('Update facility name', facility.name);
                              if (!name) return;
                              try {
                                await fetch(`/api/facilities/${facility.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token') || ''}` }, body: JSON.stringify({ name }) });
                                await fetchData();
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                await fetch(`/api/facilities/${facility.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('token') || ''}` } });
                                await fetchData();
                              } catch (e) { console.error(e); }
                            }}
      ...
      {showAddModal && activeTab === 'facilities' && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Add Facility</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={facilityForm.name} onChange={(e) => setFacilityForm({ ...facilityForm, name: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Type</label>
                  <select value={facilityForm.type} onChange={(e) => setFacilityForm({ ...facilityForm, type: e.target.value as any })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2">
                    <option value="biomethanization">Biomethanization</option>
                    <option value="wte">Waste-to-Energy</option>
                    <option value="recycling">Recycling</option>
                    <option value="composting">Composting</option>
                ...
                <div>
                  ...
                  <input type="number" value={facilityForm.capacity} onChange={(e) => setFacilityForm({ ...facilityForm, capacity: Number(e.target.value) })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">ULB ID</label>
                  <input value={facilityForm.ulbId} onChange={(e) => setFacilityForm({ ...facilityForm, ulbId: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Address</label>
                  <input value={facilityForm.address} onChange={(e) => setFacilityForm({ ...facilityForm, address: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
              </div>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      setCreating(true);
                      await apiPost('/facilities/register', {
                        name: facilityForm.name,
                        type: facilityForm.type,
                        capacity: facilityForm.capacity,
                        ulbId: facilityForm.ulbId,
                        location: { address: facilityForm.address }
                      });
                      setShowAddModal(false);
                      setFacilityForm({ name: '', type: 'biomethanization', capacity: 0, address: '', ulbId: '' });
                      await fetchData();
                    } catch (e) {
                ...
                >{creating ? 'Saving...' : 'Save Facility'}</button>
              </div>
            </div>
          </div>
        </div>
