...
const WasteManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [hhRes, bgRes] = await Promise.all([
        apiGet('/waste/households'),
        apiGet('/waste/bulk-generators')
      ]);
      const hh: Household[] = hhRes?.data || hhRes || [];
      const bg: BulkGenerator[] = bgRes?.data || bgRes || [];
      setHouseholds(hh);
      setBulkGenerators(bg);
      setLoading(false);
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      const dataset = activeTab === 'households' ? 'households' : 'bulk_generators';
      const resp = await fetch(`/api/waste/export?dataset=${dataset}`, {
        method: 'GET',
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'households'
          ...
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'bulk-generators'
                ? 'border-blue-500 text-blue-600'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <div className={`h-10 w-10 rounded-full flex items-center justify-center ${isHousehold ? 'bg-blue-100' : 'bg-green-100'
                            }`}>
                            {isHousehold ? (
