...
const ULBManagement: React.FC = () => {
    ...
    const [form, setForm] = useState({
        ...
        street: '', city: '', pincode: '', contactPhone: '', contactEmail: ''
    });
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch ULB data from API
            const [ulbsResponse, policiesResponse, metricsResponse] = await Promise.all([
                apiGet('/ulb'),
                apiGet('/ulb/policies'),
                apiGet('/ulb/metrics')
            ]);

            setUlbs(ulbsResponse.data || []);
    ...
    const handleExport = async () => {
        try {
            const response = await fetch('/api/ulb/export');
            const blob = await response.blob();
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `ulb-export-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    return (
        <div className="space-y-6">
            ...
            {showAddModal && activeTab === 'ulbs' && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                    <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
                        <div className="mt-3">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-medium text-gray-900">Register ULB</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <input placeholder="Code" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="State" value={form.state} onChange={(e) => setForm({ ...form, state: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="District" value={form.district} onChange={(e) => setForm({ ...form, district: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="Street" value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="City" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="Pincode" value={form.pincode} onChange={(e) => setForm({ ...form, pincode: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="Contact Phone" value={form.contactPhone} onChange={(e) => setForm({ ...form, contactPhone: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                                <input placeholder="Contact Email" value={form.contactEmail} onChange={(e) => setForm({ ...form, contactEmail: e.target.value })} className="border border-gray-300 rounded-md px-3 py-2" />
                            </div>
                            <div className="mt-6 flex justify-end space-x-3">
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        try {
                                            ...
                                            const resp = await fetch('/api/ulb/register', {
                                                method: 'POST',
                                                ...
                                                body: JSON.stringify({
                                                    ...
                                                    wasteManagementStatus: { totalWard: 1, activeWards: 1, totalPopulation: 1, wasteGeneratedPerDay: 0, segregationCompliance: 0 },
                                                    policies: { segregationMandatory: true, penaltyAmount: 0, incentiveAmount: 0, trainingRequired: false }
                                                })
                                            });
                                            if (!resp.ok) throw new Error('Failed to register ULB');
                                            setShowAddModal(false);
                                            setForm({ name: '', code: '', state: '', district: '', street: '', city: '', pincode: '', contactPhone: '', contactEmail: '' });
                                            await fetchData();
                                        } catch (e) {
                                            console.error('ULB registration failed', e);
                                        } finally {
                                            setCreating(false);
                                        }
                                    }}
                                    className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50"
                                >{creating ? 'Saving...' : 'Register ULB'}</button>
                            </div>
                        </div>
                    </div>
                </div>
