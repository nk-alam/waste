...
const MonitoringSystem: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [dumpingResponse, reportsResponse, cleanlinessResponse] = await Promise.all([
        apiGet('/monitoring/dumping-sites'),
        apiGet('/monitoring/community-reports'),
        apiGet('/monitoring/area-cleanliness')
      ]);

      setDumpingSites(dumpingResponse.data || []);
      setCommunityReports(reportsResponse.data || []);
      setAreaCleanliness(cleanlinessResponse.data || []);
      setLoading(false);
    } catch (error) {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `monitoring-export-${new Date().toISOString().split('T')[0]}.csv`;
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'dumping'
          ...
          <button
            ...
            className={`py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'cleanliness'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
                    ...
                    <tr key={site.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={async () => {
                              const description = prompt('Update description', site.description);
                              if (!description) return;
                              try {
                                await fetch(`/api/monitoring/report/${site.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token') || ''}` }, body: JSON.stringify({ description }) });
                                await fetchData();
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                await fetch(`/api/monitoring/report/${site.id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('token') || ''}` } });
                                await fetchData();
                              } catch (e) { console.error(e); }
                            }}
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Report Dumping Site</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={reportForm.area} onChange={(e) => setReportForm({ ...reportForm, area: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Priority</label>
                  <select value={reportForm.priority} onChange={(e) => setReportForm({ ...reportForm, priority: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2">
                    <option value="low">Low</option>
                    <option value="medium">Medium</option>
                    <option value="high">High</option>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Description</label>
                  <textarea value={reportForm.description} onChange={(e) => setReportForm({ ...reportForm, description: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Latitude</label>
                  <input value={reportForm.lat} onChange={(e) => setReportForm({ ...reportForm, lat: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Longitude</label>
                  <input value={reportForm.lng} onChange={(e) => setReportForm({ ...reportForm, lng: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Address</label>
                  <input value={reportForm.address} onChange={(e) => setReportForm({ ...reportForm, address: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Photo URL</label>
                  <input value={reportForm.photoUrl} onChange={(e) => setReportForm({ ...reportForm, photoUrl: e.target.value })} className="mt-1 w-full border border-gray-300 rounded-md px-3 py-2" />
                </div>
              </div>

              <div className="mt-6 flex justify-end space-x-3">
                <button onClick={() => setShowAddModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
                <button
                  disabled={creating}
                  onClick={async () => {
                    try {
                      ...
                      const resp = await fetch('/api/monitoring/photo-upload', {
                        method: 'POST',
                        ...
                        body: JSON.stringify({
                          ...
                          location: { lat: parseFloat(reportForm.lat), lng: parseFloat(reportForm.lng), address: reportForm.address },
                          photos: reportForm.photoUrl ? [reportForm.photoUrl] : [],
                          priority: reportForm.priority
                        })
                      });
                      if (!resp.ok) throw new Error('Failed to create report');
                      setShowAddModal(false);
                      setReportForm({ area: '', description: '', address: '', lat: '', lng: '', photoUrl: '', priority: 'medium' });
                      await fetchData();
                    } catch (e) {
                      console.error('Create monitoring report failed', e);
                    } finally {
                      setCreating(false);
                    }
                  }}
                  className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50"
                >{creating ? 'Saving...' : 'Submit Report'}</button>
              </div>
            </div>
          </div>
        </div>
