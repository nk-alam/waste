...
// @route   GET /api/waste/households
// @access  Private
router.get('/households', protect, asyncHandler(async (req, res) => {
  const { ulbId, ward, q } = req.query;
  const conditions = [];
  if (ulbId) conditions.push({ type: 'where', field: 'ulbId', operator: '==', value: ulbId });
  if (ward) conditions.push({ type: 'where', field: 'address.ward', operator: '==', value: ward });
  ...
  const filtered = q ? households.filter(h => {
    ...
    const contact = h.contactInfo?.primaryContact || '';
    const query = String(q).toLowerCase();
    return street.includes(query) || city.includes(query) || contact.includes(String(q));
  }) : households;
...
// @desc    List bulk generators
// @route   GET /api/waste/bulk-generators
// @access  Private
router.get('/bulk-generators', protect, asyncHandler(async (req, res) => {
  const { ulbId, type, q } = req.query;
  const conditions = [];
  if (ulbId) conditions.push({ type: 'where', field: 'ulbId', operator: '==', value: ulbId });
  if (type) conditions.push({ type: 'where', field: 'type', operator: '==', value: type });

  const generators = await FirestoreService.customQuery('bulk_generators', [
    ...conditions,
    { type: 'orderBy', field: 'createdAt', direction: 'desc' }
  ]);

  const filtered = q ? generators.filter(g => {
    const name = g.name?.toLowerCase() || '';
    const gtype = g.type?.toLowerCase() || '';
    const contact = g.contactInfo?.primaryContact || '';
    const query = String(q).toLowerCase();
    return name.includes(query) || gtype.includes(query) || contact.includes(String(q));
  }) : generators;
...
// @desc    Export waste datasets as CSV
// @route   GET /api/waste/export
// @access  Private (Admin, ULB Admin)
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { dataset = 'households', ulbId } = req.query;
  let collection = 'households';
  if (dataset === 'bulk_generators') collection = 'bulk_generators';

  const conditions = [];
  if (ulbId) conditions.push({ type: 'where', field: 'ulbId', operator: '==', value: ulbId });

  const items = await FirestoreService.customQuery(collection, conditions);

  let headers = [];
  let rows = [];
  if (collection === 'households') {
    headers = ['id','street','city','state','pincode','residentCount','primaryContact','wet','dry','hazardous','complianceScore','isCompliant'];
    rows = items.map(h => ([
      h.id,
      h.address?.street || '',
      h.address?.city || '',
      ...
      h.address?.pincode || '',
      h.residentCount ?? 0,
      h.contactInfo?.primaryContact || '',
      h.wasteGeneration?.dailyWetWaste ?? 0,
      h.wasteGeneration?.dailyDryWaste ?? 0,
      h.wasteGeneration?.dailyHazardousWaste ?? 0,
      h.segregationStatus?.complianceScore ?? 0,
      h.segregationStatus?.isCompliant ? 'yes' : 'no'
    ].map(v => String(v).replaceAll('"', '""'))));
  } else {
    headers = ['id','name','type','city','state','primaryContact','wet','dry','hazardous','complianceScore','isCompliant'];
    rows = items.map(g => ([
      g.id,
      g.name || '',
      g.type || '',
      g.address?.city || '',
      g.address?.state || '',
      g.contactInfo?.primaryContact || '',
      g.wasteGeneration?.dailyWetWaste ?? 0,
      g.wasteGeneration?.dailyDryWaste ?? 0,
      g.wasteGeneration?.dailyHazardousWaste ?? 0,
      g.complianceStatus?.complianceScore ?? 0,
      g.complianceStatus?.isCompliant ? 'yes' : 'no'
    ].map(v => String(v).replaceAll('"', '""'))));
  }
