...
// @desc    List green champions
// @route   GET /api/green-champions
// @access  Private
router.get('/', protect, asyncHandler(async (req, res) => {
  const { ulbId, area, q } = req.query;

  const conditions = [];
  if (ulbId) conditions.push({ type: 'where', field: 'ulbId', operator: '==', value: ulbId });
  if (area) conditions.push({ type: 'where', field: 'areaAssigned', operator: '==', value: area });

  const champions = await FirestoreService.customQuery('green_champions', [
    ...conditions,
    { type: 'orderBy', field: 'createdAt', direction: 'desc' }
  ]);

  // Simple client-side filter for q
  const filtered = q ? champions.filter(c => {
    const name = c.personalInfo?.name?.toLowerCase() || '';
    const email = c.personalInfo?.email?.toLowerCase() || '';
    const areaAssigned = c.areaAssigned?.toLowerCase() || '';
    const query = String(q).toLowerCase();
    return name.includes(query) || email.includes(query) || areaAssigned.includes(query);
  }) : champions;

  res.json({ success: true, data: filtered });
}));

// @desc    Update green champion
// @route   PUT /api/green-champions/:id
// @access  Private (Admin, ULB Admin)
router.put('/:id', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const update = req.body || {};

  const champion = await FirestoreService.getById('green_champions', id);
  if (!champion) {
    res.status(404);
    throw new Error('Green champion not found');
  }

  const updated = await FirestoreService.update('green_champions', id, {
    ...update,
    updatedAt: new Date()
  });
...
// @desc    Delete green champion
// @route   DELETE /api/green-champions/:id
// @access  Private (Admin, ULB Admin)
router.delete('/:id', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  await FirestoreService.remove('green_champions', id);
  res.json({ success: true, message: 'Green champion deleted' });
}));

// @desc    Export champions as CSV
// @route   GET /api/green-champions/export
// @access  Private (Admin, ULB Admin)
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { ulbId, area } = req.query;
  ...
  const headers = ['id','name','email','phone','areaAssigned','city','state','pincode','totalReports','resolvedReports','citizensTrained','violationsReported'];
  const rows = champions.map(c => ([
    ...
    c.address?.state || '',
    c.address?.pincode || '',
    c.performanceMetrics?.totalReports ?? 0,
    c.performanceMetrics?.resolvedReports ?? 0,
    c.performanceMetrics?.citizensTrained ?? 0,
    c.performanceMetrics?.violationsReported ?? 0
  ].map(v => String(v).replaceAll('"', '""'))));

  const csv = [headers.join(','), ...rows.map(r => r.map(v => /[,\n]/.test(v) ? `"${v}"` : v).join(','))].join('\n');
  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="green_champions.csv"');
