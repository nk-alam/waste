...
router.get('/compliance/report/:ulbId', protect, asyncHandler(async (req, res) => {
  ...
  if (format === 'pdf') {
    ...
    res.json({ success: true, message: 'PDF generation not implemented in this demo', data: complianceReport });
  } else {
    res.json({
      success: true,
...
// @desc    Export ULB facilities and summary
// @route   GET /api/ulb/:ulbId/export
// @access  Private (Admin)
router.get('/:ulbId/export', protect, authorize('admin'), asyncHandler(async (req, res) => {
  const { ulbId } = req.params;
  ...
  const headers = ['id', 'name', 'type', 'capacity', 'currentLoad', 'status'];
  const rows = facilities.map(f => [
    f.id, f.name || '', f.type || '', f.capacity ?? 0, f.currentLoad ?? 0, f.status || 'active'
  ].map(v => String(v).replaceAll('"', '""')));

  const csv = [headers.join(','), ...rows.map(r => r.map(v => /[,\n]/.test(v) ? `"${v}"` : v).join(','))].join('\n');
  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="ulb_facilities.csv"');
