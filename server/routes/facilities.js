...
// @access  Private (Admin, ULB Admin)
router.put('/:facilityId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { facilityId } = req.params;
  ...
  const updated = await FirestoreService.update('waste_facilities', facilityId, {
    ...updates,
    updatedAt: new Date()
  });
...
// @access  Private (Admin, ULB Admin)
router.delete('/:facilityId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { facilityId } = req.params;
  const facility = await FirestoreService.getById('waste_facilities', facilityId);
  if (!facility) {
    res.status(404);
  ...
  await FirestoreService.remove('waste_facilities', facilityId);
  res.json({ success: true, message: 'Facility deleted' });
}));

// @desc    Update waste intake (simple update)
// @route   PUT /api/facilities/intakes/:intakeId
// @access  Private (Admin, ULB Admin)
router.put('/intakes/:intakeId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { intakeId } = req.params;
  const updates = req.body || {};
  const intake = await FirestoreService.getById('waste_intake', intakeId);
  if (!intake) {
    res.status(404);
    throw new Error('Intake not found');
  }
  delete updates.id;
  const updated = await FirestoreService.update('waste_intake', intakeId, updates);
  res.json({ success: true, message: 'Intake updated', data: updated });
}));

// @desc    Delete intake
// @route   DELETE /api/facilities/intakes/:intakeId
// @access  Private (Admin, ULB Admin)
router.delete('/intakes/:intakeId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { intakeId } = req.params;
  const intake = await FirestoreService.getById('waste_intake', intakeId);
  if (!intake) {
    res.status(404);
  ...
  await FirestoreService.delete('waste_intake', intakeId);
  res.json({ success: true, message: 'Intake deleted', id: intakeId });
}));

// @desc    Export facilities as CSV
...
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  ...
  const headers = ['id', 'name', 'type', 'city', 'state', 'capacity', 'currentLoad', 'availableCapacity', 'status', 'managerName', 'managerPhone'];
  const rows = facilities.map(f => ([
    f.id,
    f.name || '',
    f.type || '',
    f.location?.address?.split(',')?.slice(-2, -1)[0] || f.location?.address || '',
    '',
    f.capacity ?? 0,
    f.currentLoad ?? 0,
    (f.capacity ?? 0) - (f.currentLoad ?? 0),
    f.status || 'active',
    f.manager?.name || '',
    f.manager?.phone || ''
