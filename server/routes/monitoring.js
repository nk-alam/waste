...
// @route   PUT /api/monitoring/report/:reportId
// @access  Private (Admin, ULB Admin, Supervisor)
router.put('/report/:reportId', protect, authorize('admin', 'ulb_admin', 'supervisor'), asyncHandler(async (req, res) => {
  ...
  const updated = await FirestoreService.update('monitoring_reports', reportId, {
    ...updates
  });
...
// @route   DELETE /api/monitoring/report/:reportId
// @access  Private (Admin, ULB Admin)
router.delete('/report/:reportId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { reportId } = req.params;
  const report = await FirestoreService.getById('monitoring_reports', reportId);
  if (!report) {
    res.status(404);
  ...
  await FirestoreService.delete('monitoring_reports', reportId);
  res.json({ success: true, message: 'Report deleted', id: reportId });
}));

// @desc    Export monitoring reports as CSV
// @route   GET /api/monitoring/export
// @access  Private (Admin, ULB Admin)
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { type = 'monitoring', days = 30 } = req.query;
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - parseInt(days));
  ...
  if (type === 'monitoring') {
    ...
    headers = ['id', 'area', 'issueType', 'priority', 'status', 'reportedAt'];
    rows = reports.map(r => [
      r.id, r.area || '', r.issueType || '', r.priority || '', r.status || '', new Date(r.reportedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  } else if (type === 'waste_movement') {
    const wm = await FirestoreService.customQuery('waste_movement_reports', [
      { type: 'where', field: 'reportedAt', operator: '>=', value: startDate },
      { type: 'orderBy', field: 'reportedAt', direction: 'desc' }
    ]);
    headers = ['id', 'wasteType', 'estimatedQuantity', 'status', 'reportedAt'];
    rows = wm.map(r => [
      r.id, r.wasteType || '', r.estimatedQuantity ?? 0, r.status || '', new Date(r.reportedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  } else {
    const assessments = await FirestoreService.customQuery('cleanliness_assessments', [
      { type: 'where', field: 'assessedAt', operator: '>=', value: startDate },
      { type: 'orderBy', field: 'assessedAt', direction: 'desc' }
    ]);
    headers = ['id', 'area', 'score', 'assessedAt'];
    rows = assessments.map(a => [
      a.id, a.area || '', a.score ?? 0, new Date(a.assessedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  }

  const csv = [headers.join(','), ...rows.map(r => r.map(v => /[,\n]/.test(v) ? `"${v}"` : v).join(','))].join('\n');
  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="monitoring.csv"');
