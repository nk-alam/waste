...
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  ...
  if (type === 'rewards') {
    ...
    headers = ['id', 'citizenId', 'generatorId', 'points', 'type', 'awardedAt'];
    rows = rewards.map(r => [
      r.id, r.citizenId || '', r.generatorId || '', r.points ?? 0, r.rewardType || '', new Date(r.awardedAt).toISOString()
    ...
    headers = ['id', 'citizenId', 'rewardId', 'pointsUsed', 'quantity', 'redeemedAt'];
    rows = redemptions.map(r => [
      r.id, r.citizenId || '', r.rewardId || '', r.pointsUsed ?? 0, r.quantity ?? 0, new Date(r.redeemedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  } else {
    const penalties = await FirestoreService.customQuery('penalties', [
      { type: 'where', field: 'imposedAt', operator: '>=', value: startDate },
    ...
    headers = ['id', 'citizenId', 'violationType', 'amount', 'status', 'imposedAt'];
    rows = penalties.map(p => [
      p.id, p.citizenId || '', p.violationType || '', p.amount ?? 0, p.status || 'pending', new Date(p.imposedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  }

  const csv = [headers.join(','), ...rows.map(r => r.map(v => /[,\n]/.test(v) ? `"${v}"` : v).join(','))].join('\n');
  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'attachment; filename="incentives.csv"');
