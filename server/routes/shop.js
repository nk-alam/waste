...
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  ...
  const headers = ['id', 'citizenId', 'productType', 'productName', 'quantity', 'totalPrice', 'status', 'orderedAt'];
  const rows = orders.map(o => [
    o.id, o.citizenId || '', o.productType || '', o.productName || '', o.quantity ?? 0, o.totalPrice ?? 0, o.status || '', new Date(o.orderedAt).toISOString()
