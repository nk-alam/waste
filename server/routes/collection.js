...
router.get('/vehicles', protect, asyncHandler(async (req, res) => {
  ...
  if (status) conditions.push({ type: 'where', field: 'status', operator: '==', value: status });
  if (area) conditions.push({ type: 'where', field: 'area', operator: '==', value: area });
...
// @route   PUT /api/collection/vehicles/:vehicleId
// @access  Private (Admin, ULB Admin)
router.put('/vehicles/:vehicleId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  ...
  const updated = await FirestoreService.update('collection_vehicles', vehicleId, {
    ...updates,
    updatedAt: new Date()
  });
...
// @desc    Delete vehicle
// @route   DELETE /api/collection/vehicles/:vehicleId
// @access  Private (Admin, ULB Admin)
router.delete('/vehicles/:vehicleId', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  const { vehicleId } = req.params;
  ...
  await FirestoreService.delete('collection_vehicles', vehicleId);

  res.json({ success: true, message: 'Vehicle deleted', id: vehicleId });
}));
...
router.get('/export', protect, authorize('admin', 'ulb_admin'), asyncHandler(async (req, res) => {
  ...
  if (type === 'pickups') {
    const pickups = await FirestoreService.customQuery('pickup_records', [
      { type: 'where', field: 'collectedAt', operator: '>=', value: startDate },
      { type: 'orderBy', field: 'collectedAt', direction: 'desc' }
    ]);
    headers = ['id', 'vehicleId', 'householdId', 'wasteType', 'quantity', 'quality', 'collectedAt'];
    rows = pickups.map(p => [
      p.id, p.vehicleId, p.householdId, p.wasteType, p.quantity ?? 0, p.quality || '', new Date(p.collectedAt).toISOString()
    ...
    const rejections = await FirestoreService.customQuery('pickup_rejections', [
      ...
      { type: 'orderBy', field: 'rejectedAt', direction: 'desc' }
    ]);
    headers = ['id', 'vehicleId', 'householdId', 'reason', 'rejectedAt'];
    rows = rejections.map(r => [
      r.id, r.vehicleId, r.householdId, r.reason || '', new Date(r.rejectedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  } else {
    ...
    headers = ['id', 'householdId', 'type', 'priority', 'status', 'reportedAt'];
    rows = complaints.map(c => [
      c.id, c.householdId, c.complaintType || '', c.priority || '', c.status || '', new Date(c.reportedAt).toISOString()
    ].map(v => String(v).replaceAll('"', '""')));
  }
